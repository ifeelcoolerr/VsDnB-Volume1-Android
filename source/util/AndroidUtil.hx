...
class AndroidUtil
{
	...
	{
		#if mobile
		if (!FileSystem.exists(AndroidUtil.getStorageDirectory()))
		{
                        try {
			FileSystem.createDirectory(AndroidUtil.getStorageDirectory());
                        }
                        catch (e){
                        Lib.application.window.alert('Please create folder to\n' + AndroidUtil.getStorageDirectory() + '\nPress Ok to close the app', 'Error!');
			LimeSystem.exit(1);
                        }
		}
		if (!FileSystem.exists(AndroidUtil.getStorageDirectory() + 'assets') && !FileSystem.exists(AndroidUtil.getStorageDirectory() + 'mods'))
		{
		...
		else if ((FileSystem.exists(AndroidUtil.getStorageDirectory() + 'assets') && !FileSystem.isDirectory(AndroidUtil.getStorageDirectory() + 'assets'))
			&& (FileSystem.exists(AndroidUtil.getStorageDirectory() + 'mods') && !FileSystem.isDirectory(AndroidUtil.getStorageDirectory() + 'mods')))
		{
		...
		{
			if (!FileSystem.exists(AndroidUtil.getStorageDirectory() + 'assets'))
			{
			...
			if (!FileSystem.exists(AndroidUtil.getStorageDirectory() + 'mods'))
			{
			...
			else if (FileSystem.exists(AndroidUtil.getStorageDirectory() + 'mods') && !FileSystem.isDirectory(AndroidUtil.getStorageDirectory() + 'mods'))
			{
	...
	private static function onError(e:UncaughtErrorEvent):Void
	{
		...
		try
		{
			if (!FileSystem.exists(AndroidUtil.getStorageDirectory() + 'logs'))
				FileSystem.createDirectory(AndroidUtil.getStorageDirectory() + 'logs');

			File.saveContent(AndroidUtil.getStorageDirectory()
				+ 'logs/'
			...
			fileData:String = 'you forgot to add something in your code lol'):Void
	{
		try
		{
			if (!FileSystem.exists(AndroidUtil.getStorageDirectory() + 'saves'))
				FileSystem.createDirectory(AndroidUtil.getStorageDirectory() + 'saves');

			File.saveContent(AndroidUtil.getStorageDirectory() + 'saves/' + fileName + fileExtension, fileData);
		}
		catch (e:Dynamic)
		{
			#if (android && debug)
		...
		try
		{
			if (!FileSystem.exists(savePath) && LimeAssets.exists(copyPath))
			{
				if (!FileSystem.exists(Path.directory(savePath)))
					AndroidUtil.mkDirs(Path.directory(savePath));
